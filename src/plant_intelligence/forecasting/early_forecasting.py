...
def _build_longitudinal_frame(fold_map: pd.DataFrame, phen: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for protocol in ("a", "b"):
        d15 = phen.loc[phen["phenotype_name"] == f"shoots 15d protocol {protocol}", ["accession_id", "phenotype_mean"]].rename(columns={"phenotype_mean": "x15"})
        d21 = phen.loc[phen["phenotype_name"] == f"shoots 21d protocol {protocol}", ["accession_id", "phenotype_mean"]].rename(columns={"phenotype_mean": "y21"})
...
def _fit_genomic_projection(X_train: np.ndarray, X_test: np.ndarray):
    ...
    return pca.fit_transform(X_train_scaled), pca.transform(X_test_scaled), int(pca.n_components_)


def _design(Z: np.ndarray, protocol_b: np.ndarray, x15: np.ndarray, specification: str) -> np.ndarray:
    if specification == "X15":
        return x15.reshape(-1, 1)
    if specification == "P+X15":
        return np.column_stack([protocol_b, x15])
    if specification == "G+X15":
        return np.column_stack([Z, x15])
...
def run_early_forecasting() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    specifications = ("Mean", "G", "G+P", "X15", "P+X15", "G+X15", "G+P+X15")
    predictions, fold_metrics, pca_rows = [], [], []
    ...
    for fold in sorted(frame["fold"].unique()):
        ...
        train_lookup = {a: Z_train_unique[i] for i, a in enumerate(train_accession_rows["accession_id"].astype(str))}
        test_lookup = {a: Z_test_unique[i] for i, a in enumerate(test_accession_rows["accession_id"].astype(str))}
        ...
        for specification in specifications:
            ...
            predictions.append(pd.DataFrame({
                ...
                "fold": int(fold), "specification": specification,
                "x15": x15_test, "y_true": y_test, "y_pred": y_pred,
            }))
    ...
    baseline_rmse = float(pooled.loc["Mean", "rmse"])
    baseline_mae = float(pooled.loc["Mean", "mae"])
    x15_rmse = float(pooled.loc["X15", "rmse"])
    x15_mae = float(pooled.loc["X15", "mae"])
    summary_df["rmse_improvement_vs_mean"] = np.nan
    summary_df["mae_improvement_vs_mean"] = np.nan
    summary_df["rmse_improvement_vs_X15"] = np.nan
    summary_df["mae_improvement_vs_X15"] = np.nan
    mask = summary_df["scope"] == "pooled"
    summary_df.loc[mask, "rmse_improvement_vs_mean"] = baseline_rmse - summary_df.loc[mask, "rmse"]
    summary_df.loc[mask, "mae_improvement_vs_mean"] = baseline_mae - summary_df.loc[mask, "mae"]
    summary_df.loc[mask, "rmse_improvement_vs_X15"] = x15_rmse - summary_df.loc[mask, "rmse"]
    summary_df.loc[mask, "mae_improvement_vs_X15"] = x15_mae - summary_df.loc[mask, "mae"]
    ...
    ax.set_title("Case Study A — Information ablation for Day-21 forecasting")
