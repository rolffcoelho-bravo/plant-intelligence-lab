...
class GroundedScientificInterface:
    ...
    evidence packet. A generative model can be connected later as a rendering
    layer, but it must remain downstream of this evidence boundary.
    """
    ...
    def evidence_catalog(self) -> tuple[EvidenceItem, ...]:
        ...
        champion = forecast[(forecast["specification"] == "X15") & (forecast["scope"] == "pooled")].iloc[0]

        coverage = self._read("case_study_a_uncertainty_coverage.csv")
        coverage90 = coverage[(coverage["scope"] == "pooled") & (coverage["nominal_coverage"] == 0.9)].iloc[0]
        ...
        guided10 = selection[(selection["strategy"] == "predicted_response") & (selection["budget"] == 10)].iloc[0]
        random10 = selection[(selection["strategy"] == "random") & (selection["budget"] == 10)].iloc[0]
        ...
        return (
            EvidenceItem(
                topic="forecast",
                statement="Day-15 phenotype alone is the champion Day-21 forecast in the information-ablation study.",
                values={
            ...
            EvidenceItem(
                topic="uncertainty",
                statement="The 90% conformal interval is close to nominal coverage on pooled out-of-fold predictions.",
                values={
            ...
            EvidenceItem(
                topic="abstention",
                statement="The reliability filter abstains on a small subset with much larger retrospective error.",
                values={
            ...
            EvidenceItem(
                ...
                statement="Predicted-response ranking strongly enriched high-value outcomes at budget 10 in the retrospective benchmark.",
                values={
                    "budget": 10,
            ...
            EvidenceItem(
                topic="protocol",
                statement="Protocol response is heterogeneous across accessions, with stronger evidence for a positive mean shift at Day 15 than Day 21.",
                values={
            ...
            EvidenceItem(
                topic="genomics",
                statement="Genomic-only GBLUP prediction is weak under genotype-aware validation and variance estimation is frequently boundary-limited.",
                values={
    ...
    def _select_topics(question: str) -> set[str]:
        ...
        topics: set[str] = set()
        mapping = {
            "forecast": ("forecast", "predict", "accuracy", "r2", "rmse", "day 21", "day-21"),
            "uncertainty": ("uncertainty", "interval", "coverage", "conformal", "calibration"),
            "abstention": ("abstain", "reliability", "confidence", "difficult"),
            "experiment_selection": ("experiment", "selection", "active learning", "priorit", "exploit", "explore", "budget"),
            "protocol": ("protocol", "treatment", "response heterogeneity", "delta"),
            "genomics": ("genomic", "genome", "snp", "gblup", "heritability", "genetics"),
        }
    ...
    def answer(self, question: str) -> GroundedAnswer:
        ...
        return GroundedAnswer(question=question, answer=answer, evidence=evidence, limitations=limitations)


def main() -> None:
    parser = argparse.ArgumentParser(description="Query validated Plant Intelligence Lab evidence.")
    parser.add_argument("question", help="Scientific question about the validated Case Study A results.")
    parser.add_argument(
        "--packet",
        action="store_true",
    ...
    payload = interface.build_grounding_packet(args.question) if args.packet else interface.answer(args.question).to_dict()
